...
//			the card should never move from MATCHING to another state during gameplay
...
export default class MemoryGame extends Component {
	...
	handleNewGame = () => {
		let cards = this.state.cards.map(c => ({...c,cardState: CardState.HIDING}));
		cards = shuffle(cards);
		this.setState({cards});
	}
	render(){
		const cards = this.state.cards.map((card) => (
			<Card key={card.id} showing={card.cardState !== CardState.HIDING} backgroundColor={card.backgroundColor} onClick={()=>this.handleClick(card.id)}/>
		));
